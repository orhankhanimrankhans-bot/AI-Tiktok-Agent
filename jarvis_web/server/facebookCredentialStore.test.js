...
test("Facebook credentials are isolated by stable workspace owner", async (t) => {
  ...
  assert.equal(store.list(a).length, 1); assert.equal(store.list(b).length, 0); assert.equal(store.get(id, { owner: OWNER, ...({ owner: b }) }), null); assert.equal(store.delete(id, b), false); assert.ok(store.get(id, { owner: OWNER, ...({ owner: a }) }));
});
test("Facebook credentials are encrypted, persistent, multi-account, and isolated from Google", async (t) => {
  ...
  facebook.save({ id: ids[0], accountId: "1001", accountName: "One", tokens: { userAccessToken: "facebook-secret-one" } }, OWNER);
  facebook.save({ id: ids[1], accountId: "1002", accountName: "Two", tokens: { userAccessToken: "facebook-secret-two" } }, OWNER);
  assert.equal(facebook.list(OWNER).length, 2); assert.equal((await google.list()).length, 1);
  assert.equal(Object.hasOwn(facebook.list(OWNER)[0], "tokens"), false);
  ...
  assert.equal(reopened.get(ids[1], { owner: OWNER, ...({ includeTokens: true }) }).tokens.userAccessToken, "facebook-secret-two");
...
test("manual Page credentials encrypt, update, list with OAuth, and delete without exposing tokens", async (t) => {
  ...
  store.save({ id: oauthId, accountId: "10001", accountName: "OAuth User", tokens: { userAccessToken: "oauth-plaintext-secret" } }, OWNER);
  const created = store.saveManual({ id: manualId, name: "TinyTech Facebook", pageId: "20002", pageName: "TinyTech", accessToken: "manual-plaintext-secret-one" }, OWNER);
  ...
  assert.deepEqual(new Set(store.list(OWNER).map((item) => item.authMode)), new Set(["oauth", "manual_access_token"]));
  const rowBefore = google.db.prepare("SELECT token_ciphertext FROM facebook_credentials WHERE id = ?").get(manualId);
  store.updateManual({ id: manualId, name: "Renamed Page" }, OWNER);
  ...
  assert.equal(store.get(manualId, { owner: OWNER, ...({ includeTokens: true }) }).tokens.pageAccessToken, "manual-plaintext-secret-one");
  store.updateManual({ id: manualId, name: "Renamed Page", accessToken: "manual-plaintext-secret-two" }, OWNER);
  ...
  assert.equal(store.get(manualId, { owner: OWNER, ...({ includeTokens: true }) }).tokens.pageAccessToken, "manual-plaintext-secret-two");
  assert.equal(store.delete(manualId, OWNER), true); assert.equal(store.get(manualId, { owner: OWNER }), null); assert.equal(store.get(oauthId, { owner: OWNER }).authMode, "oauth");
  await google.close();
...
test("independent Page credentials never overwrite rows sharing a Meta account or Page", async (t) => {
  ...
  store.save({ id: firstId, accountId: "meta-user-1", accountName: "Owner", pageId: "10101", pageName: "TinyTech", tokens: { userAccessToken: "user-one", pageAccessTokens: { 10101: "page-one" } } }, OWNER);
  store.save({ id: secondId, accountId: "meta-user-1", accountName: "Owner", pageId: "20202", pageName: "Page 2", tokens: { userAccessToken: "user-one", pageAccessTokens: { 20202: "page-two" } } }, OWNER);
  store.saveManual({ id: duplicatePageId, name: "Independent copy", pageId: "10101", pageName: "TinyTech copy", accessToken: "manual-page-copy" }, OWNER);
  assert.deepEqual(store.list(OWNER).map((item) => item.id), [firstId, secondId, duplicatePageId]);
  assert.deepEqual(store.list(OWNER).map((item) => item.pageName), ["TinyTech", "Page 2", "TinyTech copy"]);
});
...
test("one hundred OAuth Pages coexist and exact Page creation reuses only that opaque ID", async (t) => {
  ...
  t.after(async () => { try { await google.close(); } catch {} fs.rmSync(dir, { recursive: true, force: true }); });
  const ids = [];
  for (let index = 1; index <= 100; index += 1) { const id = FacebookCredentialStore.generateId(); ids.push(id); const pageId = String(900000 + index); store.save({ id, accountId: "shared-meta-account", accountName: "Owner", pageId, pageName: `Page ${index}`, tokens: { userAccessToken: "shared-user-token", pageAccessTokens: { [pageId]: `page-token-${index}` } } }, OWNER); }
  assert.equal(store.list(OWNER).length, 100);
  const duplicate = store.save({ id: FacebookCredentialStore.generateId(), accountId: "shared-meta-account", accountName: "Owner", pageId: "900002", pageName: "Page 2 reconnected", tokens: { userAccessToken: "updated-user-token", pageAccessTokens: { 900002: "updated-page-token" } } }, OWNER);
  assert.equal(duplicate.id, ids[1]); assert.equal(store.list(OWNER).length, 100);
});
...
test("Page-aware indexes preserve OAuth/manual separation and pending selection tokens stay encrypted", async (t) => {
  ...
  t.after(async () => { try { await google.close(); } catch {} fs.rmSync(dir, { recursive: true, force: true }); });
  const oauth = store.save({ id: FacebookCredentialStore.generateId(), accountId: "owner", pageId: "123456", pageName: "OAuth Page", tokens: { userAccessToken: "oauth-user-secret", pageAccessTokens: { 123456: "oauth-page-secret" } } }, OWNER);
  const manual = store.saveManual({ id: FacebookCredentialStore.generateId(), name: "Manual Page", pageId: "123456", accessToken: "manual-page-secret" }, OWNER);
  assert.notEqual(oauth.id, manual.id);
  const selection = store.createPageSelection({ ...({ accountId: "owner", pages: [{ id: "222222", name: "Second Page" }], tokens: { userAccessToken: "pending-user-secret", pageAccessTokens: { 222222: "pending-page-secret" } } }), sessionId: "regression-session" }, OWNER);
  assert.doesNotMatch(fs.readFileSync(dbPath).toString("latin1"), /pending-user-secret|pending-page-secret/);
  assert.equal(store.consumePageSelection({ ...({ selectionId: selection.id, pageId: "222222" }), sessionId: "regression-session" }, OWNER).page.name, "Second Page");
});
...
test("schema migration preserves existing Phase 3A OAuth records", async (t) => {
  ...
  const migrated = store.get(id, { owner: OWNER, ...({ includeTokens: true }) });
...
test("legacy-key Facebook OAuth reads without re-encryption; current-key manual credentials remain unchanged", async (t) => {
  ...
  const oauthId = FacebookCredentialStore.generateId(); const manualId = FacebookCredentialStore.generateId();
  initial.saveManual({ id: manualId, name: "Current Manual", pageId: "123456", pageName: "Page", accessToken: "current-manual-token" }, OWNER);
  ...
  assert.equal(store.get(oauthId, { owner: OWNER, ...({ includeTokens: true }) }).tokens.userAccessToken, "legacy-oauth-token");
  const oauthAfter = google.db.prepare("SELECT * FROM facebook_credentials WHERE id = ?").get(oauthId);
  assert.deepEqual(decryptTokens(oauthAfter, deriveEncryptionKey(legacySecret)), { userAccessToken: "legacy-oauth-token" });
  assert.equal(store.get(manualId, { owner: OWNER, ...({ includeTokens: true }) }).tokens.pageAccessToken, "current-manual-token");
