...
function hasPublicNumbers(metrics = {}) { return normalizeNumber(metrics.followersCount ?? metrics.followers) !== null || normalizeNumber(metrics.recentViewsTotal ?? metrics.views) !== null || normalizeNumber(metrics.postsCount ?? metrics.posts) !== null; }
...
function createFacebookControlStore({ db, now = () => new Date().toISOString(), generateId = id } = {}) {
  ...
  const latestMetrics = (pageId) => {
    const publicRow = db.prepare("SELECT followers_count AS followers, recent_views_total AS views, posts_count AS posts, posts_count AS reels, NULL AS engagement, NULL AS follower_growth, captured_at, metric_meta FROM facebook_public_metric_snapshots WHERE page_record_id = ? ORDER BY captured_at DESC LIMIT 1").get(pageId);
