...
function enforceSecurity(getStore) {
  return (req, res, next) => {
    ...
    if (!sessionIdentity(req)) return denied(res, 401, "Jarvis is locked. Authenticate to continue.");
    return hasPermission(req, permission) ? next() : denied(res, 403, "This profile does not have permission for that action.");
  };
}
function registerSecurityRoutes(app, getStore) {
  const owner = (req, res) => sessionIdentity(req)?.role === "owner" || (denied(res, 403, "Owner access is required."), false);
  app.get("/api/security/status", (req, res) => { const store = getStore(); const security = store?.publicSettings() || { enabled: false, recoveryRequired: false, autoLockMinutes: 0 }; return res.json({ security, session: security.enabled ? publicSession(req) : security.recoveryRequired ? null : { role: "owner", displayName: "Owner", permissions: OWNER_PERMISSIONS, expiresAt: null } }); });
  app.get("/api/security/public-profiles", (req, res) => { const store = getStore(); return res.json({ profiles: store?.listChildren().filter((profile) => profile.enabled).map(({ id, displayName }) => ({ id, displayName })) || [] }); });
