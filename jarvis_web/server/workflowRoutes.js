...
function registerWorkflowRoutes(app, { workflowStore, workspaceForRequest = () => ({ ownerType: "admin", ownerId: "primary" }), validateCredentialReferences = async () => true, logger = console }) {
  ...
  app.post("/api/workflows", async (req, res) => { const owner = workspace(req, res); if (!owner) return; try { const body = publicFields(req.body, CREATE_FIELDS, { required: true }); if (!await validateCredentialReferences(body.nodes, owner)) return res.status(404).json({ error: "Credential not found." }); return res.status(201).json(workflowStore.createWorkflow(body, owner)); } catch (error) { return safeError(res, error, logger); } });
  ...
  app.patch("/api/workflows/:workflowId", async (req, res) => { if (!validId(req.params.workflowId)) return res.status(400).json({ error: "Workflow ID is invalid." }); const owner = workspace(req, res); if (!owner) return; try { const body = publicFields(req.body, UPDATE_FIELDS); if (body.nodes && !await validateCredentialReferences(body.nodes, owner)) return res.status(404).json({ error: "Credential not found." }); const workflow = workflowStore.updateWorkflow(req.params.workflowId, body, owner); return workflow ? res.json(workflow) : res.status(404).json({ error: "Workflow not found." }); } catch (error) { return safeError(res, error, logger); } });
