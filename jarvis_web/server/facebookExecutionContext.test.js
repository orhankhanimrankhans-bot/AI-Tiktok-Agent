...
test("Facebook execution context keeps credential and provider wiring internal", () => {
  ...
  assert.equal(context.resolveCredential("fb_opaque", OWNER), credential); assert.deepEqual(calls, [["fb_opaque", { includeTokens: true, owner: OWNER }]]); assert.deepEqual(context.createGraphService(OWNER), { graph: true }); assert.equal(context.publishPageReel, publish); assert.deepEqual(context.resolveBinaryReference("bin_1"), { referenceId: "bin_1" }); assert.doesNotMatch(JSON.stringify({}), /secret|private/);
});
...
test("graphRequest resolves an opaque credential and creates the Graph service internally", async () => {
  ...
  const result = await context.graphRequest({ credentialId: credential.id, method: "POST", endpoint: "me", body: { credentialId: credential.id }, query: {} }, OWNER);
  assert.deepEqual(result, { id: "42", name: "Jarvis" });
  assert.deepEqual(calls, [["get", credential.id, { includeTokens: true, owner: OWNER }], ["factory"], ["me", "facebook-token-secret"]]);
...
test("graphRequest preserves the supported Page body and returns only the provider result", async () => {
  ...
  const result = await context.graphRequest({ credentialId: credential.id, method: "POST", endpoint: "page", body: { credentialId: credential.id, pageId: "123456" }, query: {} }, OWNER);
...
test("graphRequest rejects unsupported methods, endpoints, secrets, and missing credentials safely", async () => {
  ...
  await assert.rejects(context.graphRequest({ credentialId: "fcred_123", method: "GET", endpoint: "me", body: {}, query: {} }, OWNER), (error) => error instanceof FacebookGraphError && error.statusCode === 405 && !/token|private/i.test(error.message));
  await assert.rejects(context.graphRequest({ credentialId: "fcred_123", method: "POST", endpoint: "https://evil.example", body: {}, query: {} }, OWNER), (error) => error instanceof FacebookGraphError && error.statusCode === 400);
  await assert.rejects(context.graphRequest({ credentialId: "fcred_123", method: "POST", endpoint: "me", body: { accessToken: "secret" }, query: {} }, OWNER), (error) => error.publicBody?.error === "Facebook secrets must not be supplied by the client.");
  await assert.rejects(context.graphRequest({ credentialId: "fcred_123", method: "POST", endpoint: "me", body: {}, query: {} }, OWNER), (error) => error.statusCode === 404 && error.publicBody?.error === "Facebook credential was not found or is disconnected.");
});
...
test("graphRequest preserves safe provider failures without returning internal objects", async () => {
  ...
  await assert.rejects(context.graphRequest({ credentialId: credential.id, method: "POST", endpoint: "me", body: {}, query: {} }, OWNER), (error) => error instanceof FacebookGraphError && error.statusCode === 403 && error.code === "meta_200" && !/facebook-token-secret|Authorization/i.test(error.message));
});
...
test("publishReel resolves opaque credential and binary references internally before invoking the existing publisher", async () => {
  ...
  const result = await context.publishReel(request, OWNER);
  ...
  assert.deepEqual(calls.slice(0, 3), [["credential", credential.id, { includeTokens: true, owner: OWNER }], ["binary", request.binary.referenceId], ["graph"]]);
...
test("publishReel preserves safe failures and never converts a failed publisher result to publication success", async () => {
  ...
  assert.deepEqual(await context.publishReel(request, OWNER), { success: false, status: "error" });
  await assert.rejects(context.publishReel({ credentialId: credential.id, binary: null }, OWNER), (error) => error instanceof FacebookGraphError && error.statusCode === 400 && error.code === "missing_binary_reference");
  ...
  await assert.rejects(unavailable.publishReel(request, OWNER), (error) => error.statusCode === 404 && error.publicBody?.error === "Facebook credential was not found or is disconnected.");
});
...
test("publishReel validates source metadata and the browser route mapper preserves the legacy response shape", async () => {
  ...
  const internal = await context.publishReel({ credentialId: credential.id, binary: { referenceId: "bin_123" }, sourceFileId: "drive_1", sourceFileName: "source.mp4" }, OWNER);
  ...
  await assert.rejects(context.publishReel({ credentialId: credential.id, binary: { referenceId: "bin_123" }, sourceFileId: "../private" }, OWNER), (error) => error instanceof FacebookGraphError && error.code === "invalid_source_file_id");
});
...
test("publishReel propagates existing sanitized publisher failures", async () => {
  ...
  await assert.rejects(context.publishReel({ credentialId: credential.id, binary: { referenceId: "bin_123" } }, OWNER), (error) => error instanceof FacebookGraphError && error.statusCode === 422 && error.code === "reel_processing_failed" && !/facebook-token-secret|Authorization|private/i.test(error.message));
});
