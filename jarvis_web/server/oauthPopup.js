function makePopupResultHtml({ status, message = "", credentialId = null, clientUrl }) {
  ...
  return `<!doctype html>
<html>
  <head><meta charset="utf-8" /><title>Corex Google Connection</title><meta name="viewport" content="width=device-width, initial-scale=1" />
  ...
  <body>
    <div class="card"><h2>${status === "connected" ? "Google account connected" : "Google sign-in not completed"}</h2><p>${message || (status === "connected" ? "Returning to Corex…" : "You can close this window.")}</p></div>
    <script>
      (function () {
