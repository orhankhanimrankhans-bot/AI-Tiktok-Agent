...
const ACCESS_TYPES = Object.freeze(new Set(["full", "limited", "workflow", "facebook", "custom"]));
function normalizeAccessType(value, permissions) {
  const type = String(value || "").trim().toLowerCase();
  if (ACCESS_TYPES.has(type)) return type;
  const normalized = normalizePermissions(permissions);
  return PERMISSIONS.every((key) => normalized[key]) ? "full" : "limited";
}
...
function publicChild(row, includeHash = false) {
  const child = { id: row.id, displayName: row.display_name, email: row.email || "", emailRequired: !row.email, enabled: Boolean(row.enabled), accessType: row.access_type || normalizeAccessType(null, JSON.parse(row.permissions_json)), permissions: normalizePermissions(JSON.parse(row.permissions_json)), sessionLimitMinutes: row.session_limit_minutes, accessExpiresAt: row.access_expires_at || null, durationValue: row.duration_value || null, durationUnit: row.duration_unit || null, createdAt: row.created_at, updatedAt: row.updated_at };
...
class AccessControlStore {
  ...
  open() {
    ...
    if (!childColumns.has("access_type")) this.db.exec("ALTER TABLE jarvis_child_profiles ADD COLUMN access_type TEXT NOT NULL DEFAULT 'limited'");
  ...
  async createChild({ displayName, email, password, enabled = true, permissions, accessType, sessionLimitMinutes = 0, durationValue, durationUnit }) { const name = String(displayName || "").trim(); if (!name || name.length > 80) throw new Error("Display name must be 1 to 80 characters."); const normalized = normalizeEmail(email); const duplicate = this.db.prepare("SELECT id FROM jarvis_child_profiles WHERE enabled = 1 AND lower(trim(email)) = ?").get(normalized); if (duplicate) throw new Error("An active access profile already uses this email address."); const normalizedPermissions = normalizePermissions(permissions); const type = normalizeAccessType(accessType, normalizedPermissions); const nowMs = Date.now(); const now = new Date(nowMs).toISOString(); const id = `child_${crypto.randomBytes(16).toString("base64url")}`; const custom = durationUnit ? normalizeAccessDuration(durationValue, durationUnit, nowMs) : null; const minutes = Number(sessionLimitMinutes); if (!custom && ![0, 5, 15, 30, 60, 120].includes(minutes)) throw new Error("Choose a valid session duration."); this.db.prepare("INSERT INTO jarvis_child_profiles (id, display_name, email, password_hash, enabled, access_type, permissions_json, session_limit_minutes, access_expires_at, duration_value, duration_unit, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)").run(id, name, normalized, await hashPassword(password), enabled ? 1 : 0, type, JSON.stringify(normalizedPermissions), custom ? 0 : minutes, custom?.accessExpiresAt || null, custom?.durationValue || null, custom?.durationUnit || null, now, now); return this.getChild(id); }
  ...
  updateChild(id, patch) { const existing = this.getChild(id, true); if (!existing) return null; const name = patch.displayName === undefined ? existing.displayName : String(patch.displayName || "").trim(); if (!name || name.length > 80) throw new Error("Display name must be 1 to 80 characters."); const permissions = patch.permissions === undefined ? existing.permissions : normalizePermissions(patch.permissions); const enabled = patch.enabled === undefined ? existing.enabled : Boolean(patch.enabled); const email = patch.email === undefined ? existing.email : normalizeEmail(patch.email); if (enabled && !email) throw new Error("Add a valid email before enabling this access profile."); const duplicate = email && this.db.prepare("SELECT id FROM jarvis_child_profiles WHERE id <> ? AND enabled = 1 AND lower(trim(email)) = ?").get(id, email); if (duplicate) throw new Error("An active access profile already uses this email address."); const nowMs = Date.now(); const custom = patch.durationUnit ? normalizeAccessDuration(patch.durationValue, patch.durationUnit, nowMs) : null; const minutes = custom ? 0 : patch.sessionLimitMinutes === undefined ? existing.sessionLimitMinutes : safeMinutes(patch.sessionLimitMinutes); const accessExpiresAt = custom ? custom.accessExpiresAt : patch.durationUnit === null ? null : existing.accessExpiresAt; const durationValue = custom ? custom.durationValue : patch.durationUnit === null ? null : existing.durationValue; const durationUnit = custom ? custom.durationUnit : patch.durationUnit === null ? null : existing.durationUnit; const accessType = normalizeAccessType(patch.accessType || existing.accessType, permissions); this.db.prepare("UPDATE jarvis_child_profiles SET display_name = ?, email = ?, enabled = ?, access_type = ?, permissions_json = ?, session_limit_minutes = ?, access_expires_at = ?, duration_value = ?, duration_unit = ?, updated_at = ? WHERE id = ?").run(name, email || null, enabled ? 1 : 0, accessType, JSON.stringify(permissions), minutes, accessExpiresAt, durationValue, durationUnit, new Date().toISOString(), id); if (!enabled) this.revokeSessions("additional", id); return this.getChild(id); }
...
module.exports = { AccessControlStore, ADMIN_PERMISSIONS, CHILD_PERMISSIONS, DAY_MS, DEFAULT_CHILD_PERMISSIONS, OWNER_PERMISSIONS, PERMISSIONS, RESET_TTL_MS, ROLE_PERMISSIONS, hasPermission, hashPassword, normalizeAccessDuration, normalizeAccessType, normalizeEmail, normalizePermissions, normalizeRolePermissions, publicSession, safeMinutes, sessionIdentity, validStoredHash, verifyPassword, workflowWorkspace };
