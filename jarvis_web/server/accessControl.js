...
class AccessControlStore {
  ...
  deleteChild(id) { this.revokeSessions("additional", id); return this.db.prepare("DELETE FROM jarvis_child_profiles WHERE id = ?").run(id).changes > 0; }
}
function sessionIdentity(req, now = Date.now(), store = null) { const value = req.session?.jarvisAuth; if (!value?.role || (value.expiresAt && now >= value.expiresAt)) return null; if (store && value.sessionId) { const record = store.sessionRecord(value.sessionId); if (!record || record.revoked_at) return null; } if (value.role === "additional" && store) { const profile = store.getChild(value.profileId); if (!profile?.enabled || (profile.accessExpiresAt && now >= profile.accessExpiresAt)) return null; } if (value.role === "child" && store) { const account = store.childAccount(); if (!account?.enabled || account.authVersion !== value.authVersion) return null; } if (store && value.role === "admin" && store.settings()?.auth_version !== value.authVersion) return null; return value; }
...
module.exports = { AccessControlStore, ADMIN_PERMISSIONS, CHILD_PERMISSIONS, DAY_MS, DEFAULT_CHILD_PERMISSIONS, OWNER_PERMISSIONS, PERMISSIONS, RESET_TTL_MS, ROLE_PERMISSIONS, hasPermission, hashPassword, normalizeAccessDuration, normalizeEmail, normalizePermissions, normalizeRolePermissions, publicSession, safeMinutes, sessionIdentity, validStoredHash, verifyPassword };
