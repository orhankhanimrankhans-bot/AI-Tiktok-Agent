...
if (-not (Test-Path -LiteralPath $serverEntry)) { throw "Corex server entry point was not found." }
...
Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings -Principal $principal -Description "Starts the Corex Node.js backend without Chrome or VS Code." -Force | Out-Null
Start-ScheduledTask -TaskName $TaskName
Write-Output "Corex startup task installed and started: $TaskName"
