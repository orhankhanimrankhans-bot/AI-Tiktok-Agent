...
test("Child configuration is centralized and separate from Additional Access", () => { assert.match(source, /CHILD_PERMISSIONS/); assert.match(source, /\/api\/security\/child-access/); assert.match(source, /ADD USER \/ GRANT ACCESS/); assert.doesNotMatch(source, /Owner access is required\./); });
...
test("Additional Access exposes professional presets, edit controls, and archive confirmation", () => { assert.match(source, /Full Access/); assert.match(source, /Limited Access/); assert.match(source, /Edit User Access/); assert.match(source, /profile\.enabled \? "Disable"/); assert.match(source, />Delete</); assert.match(source, /Workspace data is preserved for safety/); assert.match(source, /method: "DELETE"/); assert.match(source, /setRemoveProfile\(null\)/); assert.match(source, /Create User/); });
