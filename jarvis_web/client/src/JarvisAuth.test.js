...
test("Jarvis is gated by server session restore before the private application renders", () => { assert.match(main, /<JarvisAuthGate><WorkspaceApp \/><\/JarvisAuthGate>/); assert.match(source, /\/api\/security\/status/); assert.match(source, /credentials: "include"/); });
