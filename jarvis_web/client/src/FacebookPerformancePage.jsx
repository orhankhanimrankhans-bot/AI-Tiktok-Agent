...
function metricStatus(page, key) { return page.metrics?.[key] === null || page.metrics?.[key] === undefined ? statusMessage(page) : formatCompact(page.metrics[key]); }
export function teamPerformanceRows(pages = []) {
  ...
  for (const page of pages) {
    ...
    const current = grouped.get(name) || { name, pages: 0, rawScore: 0, followers: 0, views: 0, reels: 0 };
    current.pages += 1;
    current.followers += Number(page.metrics.followers) || 0;
    current.views += Number(page.metrics.views) || 0;
    current.reels += Number(page.metrics.reels) || 0;
...
export default function FacebookPerformancePage({ apiBaseUrl = "", onBack }) {
  ...
  const load = async () => { setLoading(true); try { setState(await getFacebookPerformance(fetch, apiBaseUrl)); } catch (error) { setNotice(error.message); } finally { setLoading(false); } };
  useEffect(() => { let active = true; getFacebookPerformance(fetch, apiBaseUrl).then((data) => { if (active) setState(data); }).catch((error) => { if (active) setNotice(error.message); }).finally(() => { if (active) setLoading(false); }); return () => { active = false; }; }, [apiBaseUrl]);
