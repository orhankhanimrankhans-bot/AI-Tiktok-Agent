...
const runWorkflow = source.slice(source.indexOf("  const runWorkflow = async () =>"), source.indexOf("  const saveWorkflow = () =>"));
...
test("manual Run Workflow posts one sanitized workflow definition to the server executor", () => {
  ...
  assert.match(runWorkflow, /workflowId: "local-workflow", nodes: serverNodes, connections: serverConnections, triggerMode: "workflow"/);
