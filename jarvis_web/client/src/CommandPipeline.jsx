...
const destinationModules = ["Chat", "Voice", "WhatsApp", "TikTok", "Tools", "Memory"];

function DestinationModules() {
  return <div className="pipeline-destination-bay" aria-label="Jarvis destination modules"><span className="pipeline-bay-label">DESTINATIONS</span><div className="pipeline-module-stack">{destinationModules.map((name) => <div className="pipeline-module" key={name}><i aria-hidden="true">{name.slice(0, 2).toUpperCase()}</i><span><strong>{name}</strong><small>MODULE</small></span></div>)}</div></div>;
}

function WorkflowWires({ workflows }) {
  const count = Math.max(workflows.length, 1);
  return <svg className="workflow-wire-layer" viewBox="0 0 1000 520" preserveAspectRatio="none" aria-hidden="true"><defs><filter id="pipeline-wire-glow" x="-30%" y="-30%" width="160%" height="160%"><feGaussianBlur stdDeviation="2.4" result="blur" /><feMerge><feMergeNode in="blur" /><feMergeNode in="SourceGraphic" /></feMerge></filter></defs>{workflows.map((workflow, index) => { const y = count === 1 ? 260 : 55 + (410 * index) / (count - 1); return <g key={workflow.id} className={`wire-${workflow.dashboardStatus}`}><path className="wire-base" d={`M225 ${y} C300 ${y} 320 260 400 260`} /><path className="wire-energy" d={`M225 ${y} C300 ${y} 320 260 400 260`} /></g>; })}<g className="destination-routes">{destinationModules.map((name, index) => { const y = 55 + (410 * index) / (destinationModules.length - 1); return <g key={name}><path className="core-module-wire" d={`M480 260 C545 260 540 ${y} 610 ${y}`} /><path className="module-engine-wire" d={`M710 ${y} C770 ${y} 770 260 835 260`} /></g>; })}</g></svg>;
}
...
export default function CommandPipeline({ graph, workflows = [], activeWorkflowId = "local-workflow", workflowActive = false, workflowError = false, healthContext = {}, executions = [], onOpenWorkflow }) {
  ...
  return <section className={`command-pipeline pipeline-${state}`} data-pipeline-state={state} aria-label="Command Pipeline"><header className="command-pipeline-heading"><div><span>LIVE WORKFLOW ROUTING</span><h2>COMMAND PIPELINE</h2></div><small>{state.toUpperCase()}</small></header><div className="pipeline-live-stage"><WorkflowWires workflows={displayed} /><div className="pipeline-workflow-bay"><span className="pipeline-bay-label">WORKFLOW SOURCES</span><div className="pipeline-workflow-stack">{displayed.length ? displayed.map((workflow) => <WorkflowNode key={workflow.id} workflow={workflow} status={workflow.dashboardStatus} onOpen={onOpenWorkflow} />) : <div className="pipeline-empty-copy"><strong>No connected workflow</strong><span>Create or connect a workflow in the editor.</span></div>}</div></div><RoutingCore /><DestinationModules /><StrongEngine state={state} /></div><footer className="pipeline-routing-status"><span className="routing-status-light" />J/Route <b aria-hidden="true">-&gt;</b> <strong>{workflowActive ? "ACTIVE DATA FLOW" : workflowError ? "ATTENTION REQUIRED" : displayed.length ? "STANDBY" : "WAITING"}</strong><span className="pipeline-count-summary">{displayed.length} workflows / {graph.nodes.length} active nodes / {graph.connections.length} connections</span></footer></section>;
}
