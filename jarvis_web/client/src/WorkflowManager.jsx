...
export default function WorkflowManager({ apiBaseUrl, onClose, selectedWorkflowId, onSelectWorkflow, activeWorkflowId, onOpenWorkflow, pendingOpenWorkflowId, onCancelOpen, onDiscardAndOpen, openingWorkflowId, refreshKey, localWorkflowId, onSelectLocalWorkflow, onOpenLocalWorkflow, onDiscardAndOpenLocal }) {
  ...
  return <div className="workflow-manager-overlay" role="presentation" onMouseDown={onClose}>
    <aside className="workflow-manager" role="dialog" aria-modal="true" aria-labelledby="workflow-manager-heading" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="workflow-manager-list" aria-label="Local workflow"><button type="button" className={`workflow-manager-row workflow-manager-local${selectedLocal ? " selected" : ""}`} onClick={onSelectLocalWorkflow}><span className="workflow-manager-row-main"><strong>My Workflow</strong><small>Local browser workflow</small></span><span className="workflow-manager-status local">LOCAL</span></button></div>
