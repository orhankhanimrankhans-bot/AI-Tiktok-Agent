...
export default function WorkflowManager({ apiBaseUrl, onClose, selectedWorkflowId, onSelectWorkflow, activeWorkflowId, runningWorkflowId, onOpenWorkflow, pendingOpenWorkflowId, onCancelOpen, onDiscardAndOpen, openingWorkflowId, refreshKey, localWorkflowId, onSelectLocalWorkflow, onOpenLocalWorkflow, onDiscardAndOpenLocal }) {
  ...
  return <div className="workflow-manager-overlay" role="presentation" onMouseDown={onClose}>
    <aside className="workflow-manager" role="dialog" aria-modal="true" aria-labelledby="workflow-manager-heading" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="workflow-manager-list" aria-label="Local workflow"><button type="button" className={`workflow-manager-row workflow-manager-local${selectedLocal ? " selected" : ""}${runningWorkflowId === localWorkflowId ? " running" : ""}`} onClick={onSelectLocalWorkflow}><span className="workflow-manager-row-main"><strong>My Workflow</strong><small>Local browser workflow</small>{runningWorkflowId === localWorkflowId && <small className="workflow-manager-running-indicator"><i />RUNNING</small>}</span><span className="workflow-manager-status local">LOCAL</span></button></div>
      ...
      {selectedLocal && <footer className="workflow-manager-selection"><span>Selected</span><div className="workflow-manager-selection-summary"><strong>My Workflow</strong><span className="workflow-manager-status local">LOCAL</span></div><div className="workflow-manager-selection-actions"><button type="button" className="workflow-manager-new" onClick={onOpenLocalWorkflow}>Open Local Workflow</button></div>{pendingOpenWorkflowId === localWorkflowId && <div className="workflow-manager-discard"><p>You have unsaved changes. Open another workflow and discard them?</p><button type="button" onClick={onCancelOpen}>Cancel</button><button type="button" className="workflow-manager-new" onClick={onDiscardAndOpenLocal}>Discard and Open</button></div>}<small className="workflow-manager-selection-helper">This selection does not replace the current editor.</small></footer>}
