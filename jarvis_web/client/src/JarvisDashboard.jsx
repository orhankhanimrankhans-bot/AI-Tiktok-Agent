...
import { getFacebookControl } from "./facebookControlApi.js";
...
export default function JarvisDashboard({ apiBaseUrl = "", graph, workflowActive = false, workflowError = false, healthContext = {}, executions = [], lastExecutionAt = null, activeWorkflowId = "local-workflow", onOpenWorkflow, onOpenFacebookPages }) {
  const [detail, setDetail] = useState(null); const [conversationOpen, setConversationOpen] = useState(false); const [facebookControl, setFacebookControl] = useState({ pages: [], teams: [], sync: { refreshIntervalMinutes: 5, lastSyncAt: null } }); const [draft, setDraft] = useState(""); const [messages, setMessages] = useState([]); const [savedWorkflows, setSavedWorkflows] = useState([]); const [agentStates, setAgentStates] = useState({}); const [tasks, setTasks] = useState([]); const [activeWorkspace, setActiveWorkspace] = useState(null); const [officeHandoff, setOfficeHandoff] = useState(null); const inputRef = useRef(null); const taskSequence = useRef(0);
  ...
  const focusConversation = () => { setDetail(null); setConversationOpen(true); window.requestAnimationFrame(() => inputRef.current?.focus()); };
