...
export const THEME_PRESETS = [
  { id: "jarvis-dark", label: "Corex Dark", canvasColor: "#0d1117", canvasColorB: "#102a43", headerColor: "#171d22", accentColor: "#37d9ee" },
...
export const APPEARANCE_COLOR_SECTIONS = [
  ...
  { id: "dashboard", label: "Dashboard", fields: [["dashboardBackground", "Background"], ["dashboardPanel", "Panel Background"], ["dashboardBorder", "Panel Border"], ["dashboardHeading", "Heading"], ["dashboardText", "Body Text"], ["dashboardMuted", "Muted Text"], ["dashboardCard", "Card Background"], ["dashboardRoute", "Routing Lines"], ["dashboardCore", "Routing Core"], ["engineReady", "Engine Ready"], ["engineRunning", "Engine Running"], ["engineError", "Engine Error"], ["engineDisconnected", "Engine Disconnected"], ["conversationPanel", "Conversation Panel"], ["conversationUser", "User Message"], ["conversationJarvis", "Corex Message"], ["conversationInput", "Conversation Input"], ["dashboardIcon", "Icons"]] },
...
export const DEFAULT_APPEARANCE = { preset: "jarvis-dark", canvasStyle: "solid", gradientAngle: 135, canvasColor: "#0d1117", canvasColorB: "#102a43",
  ...
  dashboardBackground: "#070c0f", dashboardPanel: "#0d1519", dashboardBorder: "#27444e", dashboardHeading: "#effcff", dashboardText: "#c4d5da", dashboardMuted: "#71868e", dashboardCard: "#111c21", dashboardRoute: "#e1b94c", dashboardCore: "#41d9ec",
  engineReady: "#f2c94c", engineRunning: "#43e39f", engineError: "#ff5a68", engineDisconnected: "#aab4b9", conversationPanel: "#091115", conversationUser: "#17313b", conversationJarvis: "#10251f", conversationInput: "#111c21", dashboardIcon: "#69e7f6",
...
export function connectionPathToPoint(source, point) {
  const sourceX = (source.x ?? 140) + CANVAS_NODE_WIDTH;
  const sourceY = (source.y ?? 200) + CANVAS_NODE_HEIGHT / 2;
...
export function canvasPointFromClient({ clientX, clientY }, bounds, viewport) {
  return {
...
export function validateConnectionCandidate(nodes, connections, sourceId, targetId) {
  ...
  if (!source || !target) return { ok: false, error: "Choose a valid workflow input port." };
  if (sourceId === targetId) return { ok: false, error: "A node cannot connect to itself." };
  if (target.name === "Schedule Trigger") return { ok: false, error: "Schedule Trigger must remain the workflow start node." };
  if (connections.some((connection) => connection.source === sourceId && connection.target === targetId)) return { ok: false, error: "Those nodes are already connected." };
  const outgoing = new Map(nodes.map((node) => [node.id, []]));
  for (const connection of connections) outgoing.get(connection.source)?.push(connection.target);
  const seen = new Set(); const pending = [targetId];
  while (pending.length) {
    const nodeId = pending.pop();
    if (nodeId === sourceId) return { ok: false, error: "Workflow cycles are not supported." };
    if (seen.has(nodeId)) continue;
    seen.add(nodeId); pending.push(...(outgoing.get(nodeId) || []));
  }
  if (connections.some((connection) => connection.target === targetId)) return { ok: false, error: "Workflow merges are not supported in this phase." };
  if (source.name !== "Schedule Trigger" && connections.some((connection) => connection.source === sourceId)) return { ok: false, error: "Only Schedule Trigger may branch in this phase." };
