...
test("dashboard pipeline consumes real workflow state and animates only while running", () => {
  ...
  const styles = fs.readFileSync(new URL("./App.css", import.meta.url), "utf8");
  assert.match(source, /JarvisDashboard apiBaseUrl=\{API_BASE_URL\} graph=\{dashboardGraph\} workflowActive=\{isWorkflowRunning\}/); assert.match(dashboard, /technical-pipeline-board/); assert.match(dashboard, /compact-pipeline-grid/); assert.match(pipeline, /workflow-wire-layer/);
