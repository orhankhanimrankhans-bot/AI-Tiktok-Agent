...
test("Facebook node rejects Authorization and token-like fields", () => {
  assert.throws(() => assertSafeFacebookConfig({ headers: [{ name: "Authorization", value: "Bearer secret" }] }), /only on the Jarvis server/);
  assert.throws(() => assertSafeFacebookConfig({ queryParameters: [{ name: "access_token", value: "secret" }] }), /only on the Jarvis server/);
});
